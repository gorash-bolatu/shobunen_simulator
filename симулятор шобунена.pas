...
uses Cursor, MyTimers, Procs, Achievements, Inventories, Tutorial, Draw, Anim, Chat, Menu,
    Scenes, Dialogue, SlashingMinigame, ButtonMashMinigames, Matrix;
uses _Log;
...
var
    ...
    begin
        ...
        if Inv.Has(itHdd) then writeln('Ах да, ещё у тебя с собой тот жёсткий диск - его можно подключить.');
        ...
        repeat
            ...
            if (Inv.Has(itHdd) and not used_empty_hdd) then Menu.Load('подключить жёсткий диск');
            if not Inv.IsEmpty then Menu.Load('проверить инвентарь');
            ...
            case MENURES of // case begin
                ...
                {-} 'программировать ultimate alliance':
                    if programmed then writeln('Когда-нибудь ты продолжишь работу над "Ultimate Alliance". Наверное...')
                        ...
                        if (ButtonMashMinigames.ProgrammingTime < 30) then achHackerman.Achieve;
                        programmed := True;
                    end;
                {-} 'подключить жёсткий диск':
                    begin
                        ...
                        begin
                            ...
                            Anim.Next3;
                            sleep(500);
                            Matrix.Mtrx(VERSION, VERSION_nth);
                            saw_matrix := True;
                            achMatrix.Achieve;
        ...
        case CMDRES of // case begin
            {-} 'CHECK':
                begin
                    ...
                    begin
                        ...
                        if not Inv.Has(itCola) then write(' На полу валяется бутылка с колой.');
                        writeln;
                    end
                    else
                    begin
                        write('На столе ноутбук');
                        if not Inv.Has(itCola) then write(' и недопитая тобой вчерашняя бутылка колы');
                        writeln('.');
                    end;
            ...
            {-} 'BREAK_TABLE', 'JUMP_TABLE', 'THROW_TABLE':
                if broke_table then writeln('Ты уже перевернул стол!')
                    ...
                    if not Inv.Has(itCola) then writeln('Зато бутылка, вроде, осталась в порядке.');
                    broke_table := True;
                    broke_laptop := True;
                end;
            ...
            'OPEN_CODE_ALL', 'CODE_CODE_ALL':
                ...
                if charged_laptop then
                    if CMDRES.Contains('HDD') then
                        if Inv.Has(itHdd) then Computer else
                        begin
            ...
            {-} 'GET_CHARGER', 'DETECTIVE':
                if found_charger then println('Ты уже', (Inv.Has(itCharger) ? 'нашёл' : 'использовал'), 'зарядку!')
                else begin
                    TxtClr(Color.Yellow);
            ...
            'USE_CHARGER', 'USE_PC_CHARGER', 'USE_CHARGER_PC':
                ...
                else if not Inv.Has(itCharger) then writeln('Для этого понадобится найти зарядку.')
                else begin
                    Inv.Use(itCharger);
            ...
            {-} 'GET_HDD', 'GET_HDD_BOX', 'GET_BOX_HDD':
                if Inv.Has(itHdd) then writeln('Ты уже взял жёсткий диск из ящика.') else
                begin
    ...
    begin// while begin
        ...
        case CMDRES of
            ...
            'THROW_COLA', 'THROW_BOTTLE', 'THROW_COLA_BOTTLE', 'THROW_BOTTLE_COLA', 'GET_DOG_THROW_MIRROR':
                if Inv.Has(itShard) then writeln('Ты уже сломал зеркало.')
                else begin
                    ...
                    begin
                        ...
                        if Inv.Has(itCola) then println('Ты бросаешь бутылку с колой прямо в зеркало, и оно разбивается.')
                        else begin
            ...
            'OPEN_HATCH_ATTACH', 'BREAK_HATCH_ATTACH', 'CUT_HATCH_ATTACH', 'CHECK_HATCH_ATTACH':
                ...
                else if Inv.Has(itShard) then begin
